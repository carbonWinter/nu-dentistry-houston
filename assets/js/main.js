(function(global, factory) {
    ...
    const hasIntersectionObserver = window.IntersectionObserver && "isIntersecting" in IntersectionObserverEntry.prototype;
    ...
    function scrollIntoView(element, _temp) {
        ...
        return parents.reduce((fn, scrollElement, i) => {
            ...
            const {
                height: elHeight,
                top: elTop
            } = offset(parents[i - 1] || element);
            let top = Math.ceil(elTop - offset(getViewport$1(scrollElement)).top - offsetBy + scrollTop);
                    ...
                    function apply(elemOffset, targetOffset) {
                        ...
                        if (newVal >= boundary[align] && newVal + dim[prop] <= boundary[alignFlip]) {
                            ...
                            for (const el of [ "element", "target" ]) {
                                if (elemOffset) {
                                    flipped[el][dir] = flipped[el][dir] === dirs[prop][1] ? dirs[prop][2] : dirs[prop][1];
                                }
                            }
                            return true;
                        }
    ...
    function Core(UIkit) {
        ...
        let started = 0;
        on(document, "animationstart", _ref => {
            let {
                target
            } = _ref;
        ...
        methods: {
            positionAt(element, target, boundary) {
                ...
                const {
                    x,
                    y
                } = positionAt(element, target, axis === "x" ? flipPosition(this.dir) + " " + this.align : this.align + " " + flipPosition(this.dir), axis === "x" ? this.dir + " " + this.align : this.align + " " + this.dir, axis === "x" ? "" + (this.dir === "left" ? -offset$1 : offset$1) : " " + (this.dir === "top" ? -offset$1 : offset$1), null, this.flip, boundary).target;
                this.dir = axis === "x" ? x : y;
    ...
    var drop = {
        ...
        events: [ {
            ...
            handler() {
                active$1 = this;
                this.tracker.init();
                once(this.$el, "hide", on(document, pointerDown, _ref4 => {
                    let {
                        target
                    } = _ref4;
                    return !within(target, this.$el) && once(document, pointerUp + " " + pointerCancel + " scroll", _ref5 => {
                        let {
                            defaultPrevented,
                            type,
                            target: newTarget
    ...
    var formCustom = {
        ...
        update() {
            ...
            const value = input.files && input.files[0] ? input.files[0].name : matches(input, "select") && (option = $$("option", input).filter(el => el.selected)[0]) ? option.textContent : input.value;
            if (prev !== value) {
                target[prop] = value;
            }
        },
    ...
    function match$1(elements) {
        ...
        heights = elements.map((el, i) => heights[i] === max && toFloat(el.style.minHeight).toFixed(2) !== max.toFixed(2) ? "" : max);
        return {
            heights: heights,
            elements: elements
        };
    ...
    var img = {
        ...
        connected() {
            ...
            this.registerObserver(observeIntersection(this.target, (entries, observer) => {
                this.load();
                observer.disconnect();
            }, {
    ...
    function evaluateSize(size) {
        return startsWith(size, "calc") ? size.slice(5, -1).replace(sizeRe, size => toPx(size)).replace(/ /g, "").match(additionRe).reduce((a, b) => a + +b, 0) : size;
    }
        ...
        methods: {
            toggle(el, inview) {
                ...
                if (/\buk-animation-/.test(state.cls)) {
                    state.off = once(el, "animationcancel animationend", () => removeClasses(el, "uk-animation-[\\w-]+"));
                }
            ...
            read() {
                ...
                if (scrollTop === max) {
                    ...
                    this.targets.every((el, i) => {
                        if (offset(el).top - offset(getViewport$1(scrollElement)).top - this.offset <= 0) {
                            active = i;
                            return true;
                        }
                    });
    ...
    var sticky = {
            ...
            handler() {
                ...
                fastdom.read(() => {
                    ...
                    if (this.isFixed && intersectRect(targetOffset, elOffset)) {
                        scrollTop(window, targetOffset.top - elOffset.height - toPx(this.targetOffset, "height") - toPx(this.offset, "height"));
                    }
                });
            }
        } ],
        update: [ {
            read(_ref3, types) {
                ...
                const end = bottom ? bottom - offset(this.$el).height + overflow - offset$1 : getScrollingElement(this.$el).scrollHeight - windowHeight;
                return {
                    start: start,
                    end: end,
                    offset: offset$1,
        ...
        methods: {
            ...
            show(item) {
                ...
                this.connects.forEach(async _ref4 => {
                    ...
                    await this.toggleElement(toNodes(children).filter(child => hasClass(child, this.cls)), false, prev >= 0);
